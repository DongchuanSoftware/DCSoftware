...
void SVWidget::paintEvent(QPaintEvent *)
{
    ...
    for(s=0;s<=1;s+=0.01)
    {
        for(v=1;v>=0;v-=0.01)
        {
            ...
            painter.setBrush(br);
            painter.drawRect(s*width(),(1-v)*height(),width()/80,height()/80);
        }
    }
    //--------点--------
...
{
    *s_M=qme->pos().x()/(double)width();
    *v_M=1-qme->pos().y()/(double)height();
    emit parent_update();
}

void SVWidget::mouseMoveEvent(QMouseEvent *qme)
{
    if(qme->pos().x()/(double)width()<0)
    {
        *s_M=0;
    }
    else if(qme->pos().x()/(double)width()>1)
    {
        *s_M=1;
    }
    else
    ...
    if(1-qme->pos().y()/(double)height()<0)
    {
        *v_M=0;
    }
    else if(1-qme->pos().y()/(double)height()>1)
    {
        *v_M=1;
    }
    else
